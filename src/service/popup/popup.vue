  ...
  <div class="popup-container" :effect="effect"
    ...
    <div class="popup view-popup">
      ...
      <div class="popup-body" :class="{'no-content': state == 0}">
        <slot></slot>
        <button v-if="showClose == 'true'" class="button button-royal button-small button-clear button-close"
                @click="hide(-1)">
      ...
      <div v-if="buttons.length > 0" class="popup-buttons">
        ...
        <div class="liner-left" v-if="index > 0"></div>
...
<script>
  ...
  export default {
    ...
    mounted() {
      if (this.cssClass)
        this.$el.querySelector('.view-popup').classList.add(this.cssClass);
    },
