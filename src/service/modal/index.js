...
class ModalManager {
  ...
  fromComponent(com, options) {
    // click block
    if (!document.querySelector('[view-modal-click-block]')) {
      createElement('view-modal-click-block');
      let ClickBlockComponent = Vue.extend(ClickBlock);
      new ClickBlockComponent().$mount('[view-modal-click-block]');
    }
    ...
    content.$mount(modal.$el.querySelector('[view-modal-content]'));
  ...
  destroy(modal) {
    ...
    // remove click block
    if (document.querySelectorAll('[view-modal]').length == 0) {
